// ===== JavaScript: Configuração e estado global =====
	...
	// ===== JavaScript: Tabela de ranks por level =====
	// Os ranks locais foram removidos oficialmente.
	// Agora a página usa somente extensionsData.badges_levelranks.
	...
	function getCardRankData(player) {
		...
		const ranks = Array.isArray(CARD_LEVEL_RANKS) && CARD_LEVEL_RANKS.length
			? CARD_LEVEL_RANKS
			: [];
	...
	function getPlayerRoleBadgeEntries(player) {
		return Array.isArray(player?.badges?.role) ? player.badges.role.filter(function(entry) {
			return entry && typeof entry === "object" && entry.id;
		}) : [];
	}

	function getRoleBadgeDefinition(roleId) {
		return extensionsData?.badges_roles?.[roleId]?.website || extensionsData?.badges_role?.[roleId]?.website || null;
	}
	...
	function buildVerifiedCardBadgeHtml(player) {
		...
		const label = cleanValue(emblemData?.label) || "Badge Verified";
		const shortLabel = getShortVerifiedLabel(label, emblemId);
		const bio = cleanValue(emblemData?.bio || emblemData?.description || emblemData?.desc || emblemData?.info) || label;
	...
	function getPlayerClanName(player) {
		...
		value = pickClanText(raw);
		const cleaned = stripMinecraftCodes(cleanValue(value));
	...
	function buildClanInfoCardHtml(player) {
		...
		if (!clanName || !clan) {
			...
			const fallbackTitle = stripMinecraftCodes(cleanValue(fallbackClan?.label || fallbackWebsite?.title || fallbackWebsite?.label || "SEM CLA")) || "SEM CLA";
			const fallbackColor = normalizeHexColor(fallbackWebsite?.color || "#a855f7", "#a855f7");
		...
		return `
			<div class="clan-info-card clan-info-card-premium clan-info-card-tech" style="--clan-color: ${escapeHtml(color)};">
				...
				<div class="clan-info-emblem" aria-hidden="true">
					${iconHtml}
				</div>
		...
		return `
			...
			>
				...
				<div class="card-top">
					...
					${buildVerifiedCardBadgeHtml(player)}
				</div>
	...
	function createListCard(player) {
		...
		const foundClanName = getPlayerClanName(player);
		const clanName = escapeHtml(foundClanName && foundClanName !== "-" ? foundClanName : "No Clan");
		const levelText = escapeHtml(stripMinecraftCodes(getLevelText(player)));
		...
		return `
			<article
				...
				style="
					...
					--accent-soft: ${hexToRgba(color, 0.18)};
					--accent-mid: ${hexToRgba(color, 0.28)};
					--list-accent-soft: ${hexToRgba(color, 0.16)};
					--list-accent-fade: ${hexToRgba(color, 0.10)};
					--list-accent-inner: ${hexToRgba(color, 0.07)};
					--list-accent-glow: ${hexToRgba(color, 0.14)};
					--list-accent-glow-hover: ${hexToRgba(color, 0.20)};
					--list-accent-border: ${hexToRgba(color, 0.25)};
					--list-accent-border-hover: ${hexToRgba(color, 0.34)};
					--list-banner-base: ${hexToRgba(color, 0.22)};
					--list-banner-a: ${hexToRgba(color, 0.32)};
					--list-banner-b: ${hexToRgba(color, 0.18)};
				...
				<div class="list-rail"></div>
					...
					<div class="list-group1">
						...
						<div class="list-info">
							...
							<div class="list-username-line">
								<span class="list-username">${username}</span>
							</div>
							<div class="list-title">${cardTitle}</div>
						</div>
					...
					<div class="list-group2">
						...
						${buildRankTitleMarkHtml(player, "list-rank-title")}
					</div>

					<div class="list-group3">
						<div class="list-level">Lv. ${levelText}</div>
						<div class="list-clan">${clanName}</div>
					</div>
			</article>
		`;
	}
	...
	function updateViewToggleLabel() {
		...
		if (!toggleBtn) return;
		toggleBtn.textContent = isListMode ? "Cards" : "Lista";
	}
	...
	async function openDetail(playerId, sourceCard) {
		const player = playersData.find(function(item) {
			return item._id === playerId;
		});

		if (!player) return;

		await playSummonSequence(player);

		if (isListMode || !sourceCard) {
			showDetailInstant(player);
			return;
		}

		animateOpenDetail(player, sourceCard);
	}

	function closeDetail() {
		window.location.href = makeSearchUrl();
	}
	...
	async function animateOpenDetail(player, sourceCard) {
	...
	function update3DCard(card, clientX, clientY) {
		...
		const rotateY = (clampX - 0.5) * (isDetail ? 10 : 14);
		const rotateX = (0.5 - clampY) * (isDetail ? 9 : 12);

		const bgX = (clampX - 0.5) * (isDetail ? -10 : -14);
		const bgY = (clampY - 0.5) * (isDetail ? -8 : -12);

		const charX = (clampX - 0.5) * (isDetail ? -12 : -18);
		const charY = (clampY - 0.5) * (isDetail ? -6 : -8);
		...
		card.style.setProperty("--charscale", isDetail ? "1.17" : "1.08");
		card.style.setProperty("--lift", isDetail ? "-4px" : "-6px");
	...
	function attach3DEffect(cards) {
		(cards || []).forEach(function(card) {
			...
			card.addEventListener("mousemove", function(event) {
				update3DCard(card, event.clientX, event.clientY);
			});
			...
			card.addEventListener("touchstart", function(event) {
				...
				update3DCard(card, touch.clientX, touch.clientY);
			}, { passive: true });
	...
	async function load() {
		...
		try {
			const [playersRes, extensionsRes, clansRes] = await Promise.all([
				fetch(DATA_URL, { cache: "no-store" }),
				fetch(EXTENSIONS_DATA_URL, { cache: "no-store" }),
				fetch(CLANS_DATA_URL, { cache: "no-store" }).catch(function() { return null; })
			]);

			if (!playersRes.ok) throw new Error("Players HTTP " + playersRes.status + " - " + playersRes.statusText);
			if (!extensionsRes.ok) throw new Error("Extensions HTTP " + extensionsRes.status + " - " + extensionsRes.statusText);

			const playersJson = await playersRes.json();
			const extensionsJson = await extensionsRes.json();
			const clansJson = clansRes?.ok ? await clansRes.json() : {};
			...
			activePlayerId = player._id;
			document.title = stripMinecraftCodes(getDisplayName(player)) + " | Perfil";
			hideSummonBoot();
			await playSummonSequence(player);
			detailEl.innerHTML = createDetailView(player);
			detailEl.classList.add("active");
			detailEl.classList.remove("ready");
			detailEl.scrollTop = 0;
			attach3DEffect(detailEl.querySelectorAll(".detail-card"));
			setupAchievementGalleries(detailEl);
			requestAnimationFrame(function() {
	...
	function showRoute404(slug) {
		const detailEl = document.getElementById("detailView");
		if (!detailEl) return;
		document.title = "404 | Perfil não encontrado";
		detailEl.classList.add("active", "ready");
		detailEl.innerHTML = `
			<section class="route-error-page">
				<div class="route-error-card">
					<div class="route-error-code">404 / Perfil não encontrado</div>
					<h1 class="route-error-title">Não existe esse perfil</h1>
					<p class="route-error-text">
						O perfil ${slug ? `<strong>${escapeHtml(slug)}</strong>` : "informado"} não foi encontrado nos jogadores cadastrados.
					</p>
					<a class="route-error-button" href="${makeSearchUrl()}">Voltar para a busca</a>
				</div>
			</section>
		`;
	}
	...
	// ===== JavaScript: Reserva / legado (atualmente fora do fluxo principal) =====
	// Estes helpers foram mantidos por segurança, mas hoje não são chamados pela animação atual.
	function buildOpenTravelTarget(sourceRect) {
		const vw = window.innerWidth;
		const vh = window.innerHeight;

		const maxWidth = Math.min(420, vw * 0.42);
		const ratio = sourceRect.height / sourceRect.width;
		const width = Math.max(sourceRect.width + 50, maxWidth);
		const height = width * ratio;

		return {
			left: (vw - width) / 2,
			top: Math.max(60, (vh - height) / 2 - 10),
			width: width,
			height: height
		};
	}

	function buildCloseTravelTarget(sourceRect) {
		const vw = window.innerWidth;
		const vh = window.innerHeight;

		const width = Math.min(360, vw * 0.34);
		const height = width * (sourceRect.height / sourceRect.width);

		return {
			left: (vw - width) / 2,
			top: Math.max(70, (vh - height) / 2 - 12),
			width: width,
			height: height
		};
	}
	...
	document.getElementById("pageSizeSelect").addEventListener("change", function(event) {
		...
		itemsPerPage = [3, 6, 9].includes(selectedPageSize) ? selectedPageSize : 6;
		currentPage = 1;
		render(filteredPlayers);
	});
