...
// ===== JavaScript: Configuração e estado global =====
	...
	// ===== CORREÇÃO 02: remover sidebar/voltar legados mesmo se algum patch antigo recriar =====
	function removeLegacyChrome() {
		document.querySelectorAll('.archive-side-hud, .gacha-sidebar, .archive-sidebar, .category-sidebar, #backBtn, .back-btn').forEach(function(el) {
			el.remove();
		});
	}
	removeLegacyChrome();
	new MutationObserver(removeLegacyChrome).observe(document.documentElement, { childList: true, subtree: true });
