...
// ===== JavaScript: Configuração e estado global =====
	...
	async function openDetail(playerId, sourceCard) {
		...
		const player = playersData.find(function(item) {
			return String(item._id) === String(playerId);
		});
		...
		try {
			...
			if (player && sourceCard) {
				// Modo Cards mantém a animação especial. A Lista abre direto para não misturar comportamento.
				await animateOpenDetail(player, sourceCard, profileUrl);
				return;
			}
		} catch (e) {
			console.warn('[profile opening animation]', e);
		}
	...
	// ===== PERFORMANCE PATCH 2025 =====
	// Removido: loops requestAnimationFrame contínuos que atualizavam gradientes em todos os frames.
	// Agora os nomes continuam com gradiente visual via CSS estático, sem consumir CPU/GPU o tempo todo.
	function normalizeStaticGradients() {
		document.querySelectorAll(".title-gradient, .list-displayname.gradient, .list-displayname.gradient .list-displayname-text").forEach(function(el) {
	...
	document.addEventListener("DOMContentLoaded", normalizeStaticGradients, { once: true });
	setTimeout(normalizeStaticGradients, 800);
