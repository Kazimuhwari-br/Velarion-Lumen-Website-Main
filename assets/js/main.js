  ...
  ready(function(){
    ...
    document.querySelectorAll('[data-support-form]').forEach(function(form){
      form.addEventListener('submit', function(event){
        ...
        alert('Ticket visual preparado: ' + subject + '\nConecte este formulário depois com Discord, Firebase ou backend.');
      });
    });
...
// ===== JavaScript: Configuração e estado global =====
	...
	const VL_HABITANT_DATA_URLS = [
		VL_CUSTOM_DATA_URL,
		"https://kazimuhwaribedrock-default-rtdb.firebaseio.com/habitantes.json",
		"https://kazimuhwaribedrock-default-rtdb.firebaseio.com/npcPlayers.json",
		"https://kazimuhwaribedrock-default-rtdb.firebaseio.com/NPCPlayers.json",
		VL_EXTENSIONS_BASE_URL + "/habitantes.json",
		VL_EXTENSIONS_BASE_URL + "/npcPlayers.json",
		VL_EXTENSIONS_BASE_URL + "/npcs.json"
	].filter(Boolean);
	...
	function createCard(player, index) {
		...
		const clanName = getPlayerClanName(player) || "VLS";
		...
		return `
			...
			>
				...
				<div class="card-top">
					...
					<div class="card-chip card-clan-chip">
	...
	async function fetchJsonSafe(url) {
		...
		try {
			...
			console.warn("[Firebase tentativa falhou]", url, error);
			return null;
		}
	}
	...
	function renderHubPanel() {
		...
		if (!ranked.length) {
			root.innerHTML = '<div><b>--</b><strong>Nenhum dado carregado</strong><small>Confira o caminho do Firebase.</small></div>';
			return;
		}
	...
	async function load() {
		...
		try {
			setStatus("Conectando ao Firebase...", "Carregando dados reais da comunidade.");
			...
			const [dataResult, extensionsJson, clansJson] = await Promise.all([dataResultPromise, extensionsPromise, clansPromise]);

			extensionsData = extensionsJson || {};
			clansData = normalizeClans(clansJson);
			applyExtensionDataConfig();
			playersData = normalize(dataResult.json);
			filteredPlayers = playersData.slice();

			if (!playersData.length) {
				setStatus("Sem registros encontrados", "A conexão abriu, mas nenhum registro útil foi encontrado para esta página.");
				if (playersEl) playersEl.innerHTML = `<div class="empty">Nenhum registro encontrado no caminho configurado da Firebase.</div>`;
				updateCounters();
				renderHubPanel();
				return;
			}

			setStatus("Codex sincronizado", "Dados reais carregados de " + (dataResult.url || "Firebase") + ".");
			...
			setStatus("Erro ao carregar Firebase", String(e.message || e));
			if (playersEl) {
