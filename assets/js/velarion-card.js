...
(function(window, document) {
  ...
  function setData(data) {
    ...
    /* ================================================================
       ...
       - novo:   "card_color": { "cc_id1": "#...", "cc_id2": "#...", ... }

       Não existe limite artificial para cc_idN. Qualquer chave que corresponda
    ...
    function normalizeCardColorSpeed(value, fallback = 10) {
      ...
      /* cc_speed é a duração, em segundos, de um ciclo completo.
         Limites apenas evitam 0/NaN e animações impraticáveis. */
      return Math.min(300, Math.max(0.25, parsed));
    }

    function normalizeCardColorConfig(value, fallbackColor = "#ff84cf") {
      const safeFallback = isValidHexColor(fallbackColor)
        ? getCleanText(fallbackColor)
        : "#ff84cf";

      /* Formato antigo continua funcionando exatamente como antes. */
      if (typeof value === "string") {
        const color = isValidHexColor(value) ? getCleanText(value) : safeFallback;
        return {
          legacy: true,
          colors: [color],
          primary: color,
      ...
      const indexedColors = Object.entries(value)
        .map(([key, color]) => {
          ...
          if (!match || !isValidHexColor(color)) return null;
          return {
            index: Number(match[1]),
            color: getCleanText(color)
          };
        })
        .filter(Boolean)
      ...
      if (!colors.length) {
        ...
        colors.push(objectFallback ? getCleanText(objectFallback) : safeFallback);
      }
    ...
    function buildCardPaletteGradient(colors, angle = "135deg") {
      const palette = Array.isArray(colors) && colors.length ? colors : ["#ff84cf"];
      if (palette.length === 1) {
        return `linear-gradient(${angle}, ${palette[0]}, ${palette[0]})`;
      }
    ...
    function buildCardPaletteLoopGradient(colors, angle = "90deg") {
      const palette = Array.isArray(colors) && colors.length ? colors : ["#ff84cf"];
    ...
    function hexColorToRgb(value) {
      const raw = getCleanText(value).replace(/^#/, "");
      const hex = raw.length === 3
        ? raw.split("").map((char) => char + char).join("")
        : raw;

      if (!/^[0-9a-f]{6}$/i.test(hex)) {
        return { r: 255, g: 132, b: 207 };
      }
    ...
    function findClanPlayer(clanId, clanDataSource) {
      ...
      const clanColorConfig = normalizeCardColorConfig(rawColor, "#F54927");
      const color = clanColorConfig.primary;
    ...
    function normalizeCardProfile(id, data, position, requestedCharacterSlotId = "id_1") {
      ...
      // A moldura/efeitos da carta usam theme.card_embed.card_color.
      // Aceita tanto a string antiga quanto o novo objeto cc_idN sem limite.
  ...
  function applyDynamicCardColor(card, color) {
    if (!card || !isValidHexColor(color)) return;

    card.style.setProperty("--card-color", color);
    card.style.setProperty("--card-glow", color);
    card.style.setProperty("--accent", color);
    card.style.setProperty("--accent-strong", `color-mix(in srgb, ${color} 82%, #ffffff 18%)`);
    card.style.setProperty("--accent-light", `color-mix(in srgb, ${color} 22%, #ffffff 78%)`);
    card.style.setProperty("--card-color2", `color-mix(in srgb, ${color} 34%, #ffffff 66%)`);

    /* O chip acompanha a cor dinâmica da carta. */
    card.style.setProperty("--chip-rank-color", color);
    card.style.setProperty("--chip-rank-color2", `color-mix(in srgb, ${color} 34%, #ffffff 66%)`);
    card.style.setProperty("--chip-rank-glow", color);
    card.style.setProperty("--chip-rank-light", `color-mix(in srgb, ${color} 22%, #ffffff 78%)`);
  }

  function getCardRuntimePalette(card) {
    return String(card?.dataset?.cardColorPalette || "")
      .split(",")
  ...
  function updateAnimatedCardColors(now) {
    ...
    cardColorRuntime.cards.forEach((card) => {
      if (!card || !card.isConnected) {
        cardColorRuntime.cards.delete(card);
        return;
      }
      ...
      if (type === "rotate") {
        const lastIndex = Number(card.dataset.cardColorRuntimeIndex ?? -1);
        if (lastIndex !== baseIndex) {
          card.dataset.cardColorRuntimeIndex = String(baseIndex);
          applyDynamicCardColor(card, colors[baseIndex]);
        }
        return;
      }

      /* pulse = transição suave contínua entre todos os cc_idN. */
      const eased = localProgress * localProgress * (3 - 2 * localProgress);
      applyDynamicCardColor(
        card,
        interpolateHexColor(colors[baseIndex], colors[nextIndex], eased)
      );
    });

    if (active || cardColorRuntime.cards.size) {
  ...
  function setupCardColorEffects(root) {
    ...
    scope.querySelectorAll(".vl-card[data-card-color-type]").forEach((card) => {
      if (card.dataset.vlCardColorBound === "true") return;
      card.dataset.vlCardColorBound = "true";
    ...
    if (cardColorRuntime.cards.size && !cardColorRuntime.raf) {
