        ...
        <div class="vl-profile-layout">
          <aside class="vl-profile-identity">
            <article class="detail-card vl-profile-card" aria-label="${h.escapeHtml(displayNamePlain)}" style="--accent:${color};--vp-accent:${color};">
              ...
              <div class="vl-profile-card-frame" aria-hidden="true">
                <i class="vl-profile-card-corner vl-profile-card-corner--tl"></i>
                <i class="vl-profile-card-corner vl-profile-card-corner--tr"></i>
                <i class="vl-profile-card-corner vl-profile-card-corner--bl"></i>
                <i class="vl-profile-card-corner vl-profile-card-corner--br"></i>
                <span class="vl-profile-card-scan"></span>
              </div>

              <div class="vl-profile-card-base" aria-label="Resumo visual do perfil">
                <div class="vl-profile-card-base-mark" aria-hidden="true">
                  <img src="${h.escapeHtml(avatar)}" alt="" loading="eager" referrerpolicy="no-referrer" crossorigin="anonymous" onerror="this.onerror=null; this.style.display='none'; this.parentElement.dataset.fallback='${h.escapeHtml(getInitials(displayNamePlain, 'VL'))}';">
                </div>
                <div class="vl-profile-card-base-copy">
                  <small>Perfil exibido</small>
                  <strong>${h.escapeHtml(displayNamePlain)}</strong>
                  <span>${h.escapeHtml(cardTitle)}</span>
                </div>
                <div class="vl-profile-card-base-chips" aria-label="Dados rápidos">
                  <b>${h.escapeHtml(getInitials(clanName, "VL"))}</b>
                  <b>Lv. ${h.escapeHtml(level || 0)}</b>
                  <b>${h.escapeHtml(getInitials(country, "BR"))}</b>
                </div>
              </div>
            </article>
